<template>
  <div>
    <div class="main p-20">
      ...
      <el-table
        :data="delarr.slice((currentpage - 1)*pagesize,currentpage*pagesize)"
        style="width: 100%"
      >
...
export default {
  ...
  methods: {
    ...
    deleteRow(index,row) {
      ...
      this.delarr =  this.delarr.filter(row =>{
        return  row.GOODS_SERIAL_NUMBER !== num
      })
      this.tableData = this.delarr.filter(row =>{
        return  row.GOODS_SERIAL_NUMBER !== num
      })
      return 
